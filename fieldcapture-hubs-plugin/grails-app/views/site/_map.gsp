...
function initSiteViewModel() {
    ...
    function refreshGazInfo(){
        ...
        var geom = siteViewModel.extent().geometry();
        var lat, lng;
        if (geom.type === 'Point') {
            lat = siteViewModel.extent().geometry().decimalLatitude();
            lng = siteViewModel.extent().geometry().decimalLongitude();
        }
        else if (geom.centre !== undefined) {
            lat = siteViewModel.extent().geometry().centre()[1];
            lng = siteViewModel.extent().geometry().centre()[0];
        }
        else {
        ...
        .done(function(data) {
            if(data.results.length > 0){
              ...
              siteViewModel.extent().geometry().locality(data.results[0].formatted_address);
            }
        });
    }
